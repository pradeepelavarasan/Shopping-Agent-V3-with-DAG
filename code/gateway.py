...
def ensure_gateway() -> None:
    ...
    # Launch using the virtual environment's python directly to bypass uv startup check delays
    python_bin = GATEWAY_V8_DIR / ".venv" / "bin" / "python"
    cmd = [str(python_bin), "main.py"] if python_bin.exists() else ["uv", "run", "main.py"]
