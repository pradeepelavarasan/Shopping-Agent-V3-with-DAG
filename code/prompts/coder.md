...
Coding Rules:
  ...
  - Shopping Query Rule: If you are sorting or filtering products for a product search/shopping query, your Python script MUST output the final selected products as a JSON-serialized list of dictionaries (using `json.dumps`), preserving all original keys (specifically `id`, `title`, `price`, `rating`, `reviews_count`, and `url`) so downstream nodes can access the URLs. Crucially, to conform with the UI schema, the script MUST map the `id` of the selected products to "prod_1", "prod_2", and "prod_3" respectively (in order of their selection/output) before printing.
