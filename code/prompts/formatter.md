...
Rules:
  ...
  - If the USER_QUERY asks for a specific JSON format (e.g., the Shopping Agent JSON format containing "products", "analysis", and "task"), you MUST generate that exact JSON object and output it as the string value of "final_answer". Do not wrap it in markdown code fences or add any surrounding text. To do this, merge:
    1. The product details (id, title, price, rating, reviews_count, image_url, url) from the shortlister output. The `image_url` field from the shortlister contains the real product thumbnail — use it directly. Do NOT replace it with anything from the product_analyst nodes.
    2. The recommendation summary and evaluations from the product_recommendation node.
    3. The "task" priorities array matching the evaluation categories used.
