...
DEFAULT_ORDER = [
    "gemini_lite_1", "gemini_lite_2", "gemini_lite_3", "gemini_lite_4",
    "gemini_lite_5", "gemini_lite_6", "gemini_lite_7",
    "gemini_flash", "nvidia", "ollama", "groq", "cerebras", "openrouter", "github"
]
...
TINY_ENV = os.getenv("TINY_ORDER", "")
LARGE_ENV = os.getenv("LARGE_ORDER", "")

TIER_TO_ORDER = {
    "TINY":  [x.strip() for x in TINY_ENV.split(",") if x.strip()] if TINY_ENV else DEFAULT_TINY,
    "LARGE": [x.strip() for x in LARGE_ENV.split(",") if x.strip()] if LARGE_ENV else DEFAULT_LARGE,
}
