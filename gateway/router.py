...
LIMITS = {
    ...
    "gemini_flash":{"rpm": 5,   "rpd": 20,      "tpm": 250000,   "cooldown": 12,  "max_ctx": 1000000},
    "gemini_lite": {"rpm": 15,  "rpd": 500,     "tpm": 250000,   "cooldown": 4,   "max_ctx": 1000000},
...
for _i in range(1, 21):
    LIMITS[f"gemini_lite_{_i}"] = LIMITS["gemini_lite"]

SHORTCUTS = {
    "gf": "gemini_flash", "gemini_flash": "gemini_flash",
    "gl": "gemini_lite", "gemini_lite": "gemini_lite",
    "g": "gemini_lite", "gem": "gemini_lite", "gemini": "gemini_lite",
...
class Router:
    ...
    def candidates(self, override=None):
        if override:
            r = resolve(override)
            if r == "gemini_lite":
                matches = [p for p in self.providers if p.startswith("gemini_lite_")]
                if matches:
                    return matches
